...
Ces notes de publication fournissent des informations sur les nouvelles fonctionnalités, mises à jour de sécurité et mises à jour non relatives à la sécurité incluses dans les mises à jour du Canal semestriel vers Office 365 ProPlus en 2018. 
...
> - Le Canal semestriel était nommé Canal différé avant janvier 2018.

## <a name="version-1803-october-9"></a>Version 1803  : le 9 octobre
...
## <a name="version-1708-september-11"></a>Version 1708 : Septembre 11
...
## <a name="version-1803-august-14"></a>Version 1803 : le 14 août
*Version 1803 (build 9126.2275)*

### <a name="access-security-updates"></a>Access : mises à jour de sécurité
-   [CVE-2018-8312](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8312) : vulnérabilité d’exécution de code à distance Microsoft Access « Use-After-Free »
...
-   [CVE-2018-8312](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8312) : Vulnérabilité « Use-After-Free » de l’exécution de code à distance Microsoft Access 
...
-   **Interface utilisateur de partage de fichiers :** pour les fichiers OneDrive Entreprise ou SharePoint, cliquez sur le bouton Partager dans le coin supérieur droit du ruban ou accédez à Fichier \> Partager permet de démarrer une boîte de dialogue de partage simplifiée et améliorée. Pour les nouveaux fichiers ou les fichiers enregistrés localement, l’interface utilisateur permet aux utilisateurs de charger facilement leurs fichiers dans OneDrive pour commencer à collaborer.
...
-   [CVE-2018-8147](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-8147) : Vulnérabilité de l’exécution de code à distance Microsoft Excel
...
-   [CVE-2018-8248](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0907) : vulnérabilité de l’exécution de code à distance Microsoft Excel
-   [CVE-2018-0907](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0907) : Contournement de la fonctionnalité de sécurité Microsoft Office Excel
...
-   Résolution d’un problème : les actions de création de graphique pouvaient entraîner la fermeture d’Excel.
...
-   Résolution d’un problème : lorsque l’utilisateur tentait d’insérer un objet dans un classeur existant, Excel se bloquait quand l’utilisateur cliquait sur Parcourir.
...
-   Résolution d’un problème : l’info-bulle était mal alignée lorsque vous faisiez glisser un élément ou que vous remplissiez une zone par glisser.
...
-   **Téléchargement de pièces jointes dans le cloud :** lorsque vous enregistrez des pièces jointes OneDrive ou effectuez un glisser-déposer de ces dernières sur votre ordinateur, nous téléchargeons le fichier pour vous.
-   **Sons utiles améliorant l’accessibilité :** activez un indicateur sonore pour vous guider lors de votre tâche. Accédez à Fichier \> Options \> Options d’ergonomie. Aucun complément n’est nécessaire. [En savoir plus](https://support.office.com/article/49fda9be-cce5-4c72-a87f-b11000197f5f)
...
-   **Accéder rapidement aux groupes les plus utilisés :** les groupes avec lesquels vous êtes le plus susceptible d’interagir sont maintenant affichés en haut de la liste sous Groupes dans le volet Dossiers.
...
-   Résolution d’un problème : tous les participants recevaient des mises à jour de réunion lors de la modification du corps ou des pièces jointes de la réunion. Cette fonction devait normalement être facultative.
-   Résolution d’un problème : des utilisateurs ne pouvaient pas se connecter aux points de terminaison REST et EWS en raison d’une modification dans la chaîne User-Agent.
...
-   **Vue Tableau des tâches :** triez des tâches sur des cartes dans la vue Tableau des tâches. Réorganisez et déplacez les cartes entre des colonnes sur le tableau, comme dans les projets Agile.
...
-   **Gérez une tâche dans le Planificateur :** associez une tâche de projet au Planificateur et créez un plan qui lui est propre. Divisez la tâche en sous-tâches, ajoutez une équipe, attribuez des tâches et gérez le travail dans un tableau de tâches.
...
-   Résolution d’un problème : dans l’affichage Chronologie, au niveau de la boîte de dialogue « Ajouter les tâches existantes à la chronologie », uniquement les tâches de la première tâche récapitulative apparaissaient.
...
-   Résolution d’un problème : Project se bloquait lors de l’utilisation de la fonctionnalité Séquence des Tâches.
-   Résolution d’un problème : l’échelle de temps n’affichait pas les étiquettes d’échelle de temps.
-   Résolution d’un problème : les rapports visuels affichaient des informations incomplètes ou échouaient complètement.
...
-   Résolution du problème lié au blocage de Project lorsque vous accédez à des rapports qui contiennent plusieurs images.
...
-   Informez les utilisateurs que les appels et les conversations sont envoyés aux équipes lorsque ChatDefaultClient et CallDefaultClient sont définis sur la valeur Teams.
...
-   Résolution d’un problème : les utilisateurs se trouvant dans la salle d’attente n’étaient pas avertis que leur accès à la réunion avait été refusé.
...
-   **Syntaxe LaTeX :** créez et modifiez des équations mathématiques en utilisant la syntaxe LaTeX.
-   **Sons utiles améliorant l’accessibilité :** activez un indicateur sonore pour vous guider lors de votre tâche. Accédez à Fichier \> Options \> Options d’ergonomie. Aucun complément n’est nécessaire. [En savoir plus](https://support.office.com/article/49fda9be-cce5-4c72-a87f-b11000197f5f)
-   **Emplacements des fichiers par compte :** lors de l’ouverture ou de l’enregistrement d’un fichier, la liste des emplacements est organisée selon le compte qui lui est associé.
-   **Personnalisation du stylet :** choisissez un jeu personnel de stylets et de surligneurs pour l’entrée manuscrite. Votre jeu personnalisé est disponible sur tous vos ordinateurs Windows.
...
-   Résolution d’un problème : Word ne s’ouvrait pas sur un ordinateur exécutant Windows 7 où n’était pas installée la mise à jour de télémétrie de diagnostic et d’expérience client.
...
-   Résolution d’un problème : le zoom et la mise à l’échelle ne fonctionnaient pas correctement dans les compléments Office dans un environnement PPP dynamique.
...
### <a name="access-non-security-updates"></a>Access : mises à jour non relatives à la sécurité
-   Résolution d’un problème : lorsque vous utilisez un formulaire à plusieurs éléments, l’ajustement de la position de la molette de la souris ou de la barre de défilement ne modifie pas les éléments affichés dans le formulaire.
...
-   **Insertion d’images en ligne :** la nouvelle page d'arrivée pour la sélection d’images et d’informations d’attribution est automatiquement insérée avec l’image.
...
-   [CVE-2017-11877](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11877) : Vulnérabilité de contournement de la fonctionnalité de sécurité de Microsoft Excel
-   [CVE-2017-11878](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11878) : Vulnérabilité de corruption de mémoire de Microsoft Excel
-   [CVE-2017-11884](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-11884) : vulnérabilité de corruption de mémoire de Microsoft Office
...
-   Résolution d’un problème : lors de l’ouverture d’un classeur par un double-clic, les formats et les polices du texte des cellules n’étaient pas chargés ou deux classeurs identiques étaient ouverts pour un seul modèle.
...
-   Résolution d’un problème : l’info-bulle était mal alignée lorsque vous faisiez glisser un élément ou que vous remplissiez une zone par glissement.
-   Résolution d’un problème : lorsque vous enregistrez un classeur en sélectionnant Fichier \> Enregistrer sous, un nom de fichier qui contient des points apparaît vide ou tronqué dans la boîte de dialogue d’enregistrement de fichiers.
...
-   Résolution d’un problème : des en-têtes et des lignes noires apparaissaient en raison d’un pilote graphique défectueux.
...
-   Résolution d’un problème : lorsque vous choisissiez la commande Remplacer et que la boîte de dialogue Rechercher et remplacer s’ouvrait, le focus de la boîte de dialogue se trouvait sur l’onglet Rechercher au lieu de l’onglet Remplacer.
...
-   Résolution d’un problème : Excel se bloquait après l'utilisation du bouton Formulaires dans un classeur déjà fermé.
...
-   Résolution d’un problème : la modification d'une feuille de calcul était impossible lorsqu'elle comportait un tableau croisé dynamique avec des champs dans la zone de filtre, mais aucun autre champ ailleurs.
...
-   **Configuration de nouveaux comptes :** vous pouvez configurer de nouveaux comptes avec un nouvel assistant qui nécessite moins d’étapes manuelles.
...
-   **Secteurs d’activité pour les groupes :**  un niveau de secteur d’activité défini par l’administrateur des clients, par exemple Confidentiel, peut être affecté lorsque vous créez ou modifiez un groupe, et le secteur s’affiche dans l’en-tête de groupe.
...
### <a name="outlook-non-security-updates"></a>Outlook : mises à jour non relatives à la sécurité
-   Résolution d’un problème : l’utilisateur voyait la sélection dans la liste de messages se déplacer de manière inattendue lors de la suppression des messages.
...
-   Résolution d’un problème : Marquer comme lu et Marquer comme non lu apparaissaient sous forme d'options pour les messages de la boîte de réception partagée d’un groupe.
-   Résolution d’un problème : il était impossible de configurer un compte IMAP dans Outlook.
-   Résolution d’un problème : un blocage intermittent se produisait lors de l’ouverture d’Outlook.

### <a name="powerpoint-feature-updates"></a>PowerPoint : mises à jour de fonctionnalité
-   **Insertion d’images en ligne :** la nouvelle page d’arrivée pour la sélection d’images et d’informations d’attribution est automatiquement insérée avec l’image.
-   **Sous-titres pour les vidéos :** ajoutez des sous-titres à une vidéo pour étendre sa visibilité. [Plus d’informations](https://support.office.com/article/a16745e1-b3da-4428-b2a7-ff0c8b758d0b)
...
-   Résolution d’un problème : les caractères définis par l’utilisateur final (EUDC) liés aux polices ne s’affichaient pas.
...
-   Résolution d’un problème : la date de début effective pouvait être effacé à tort lors de la définition du travail restant.
-   Résolution d’un problème : la date de début effective de l’affectation pouvait être différente de celle indiquée par la ressource au moyen de la définition d’états dans l'application web de Project.
-   Résolution d’un problème : le travail réel pouvait être replanifié si la date de fin de la tâche était modifiée.
-   Résolution d’un problème : lors de la copie et du collage de champs Coûts, les valeurs collées ne correspondaient pas exactement aux valeurs copiées en raison d’un problème d’arrondi.
-   Résolution d’un problème : les données chronologiques pour les ressources budgétaires n'étaient pas copiées lorsque vous enregistriez à partir d’une ligne de base dans une autre.
-   Résolution d’un problème : le champ d’état n’effectuait pas toujours un calcul exact pour les tâches récapitulatives.
...
-   Résolution d’un problème : le projet se bloquait si la première colonne d’un de vos tableaux correspondait au nom de la tâche, et que vous cliquez sur une tâche alors que la colonne était verrouillée.
-   Résolution d’un problème : la même ressource pouvait être attribuée plusieurs fois à la même tâche dans l’affichage Utilisation des tâches.
-   Résolution d’un problème : les valeurs dans les champs personnalisés de nombre pouvaient disparaître lors de l’ouverture de fichiers XML.
-   Résolution d’un problème : la mise en retrait d’une tâche de niveau supérieur ne se synchronisait pas correctement depuis Project vers la liste des tâches SharePoint.
-   Résolution d’un problème : si vous importiez une tâche, une ressource ou des informations d’affectation à partir d’un classeur Excel, les valeurs dans le champ de travail pouvaient être ignorées.
...
-   Résolution d’un problème : le client Project n'ouvrait pas un projet car il pensait que le projet était extrait, alors qu’il ne l’était pas en réalité.
-   Résolution d’un problème : l’ouverture de fichiers Project à partir d’un serveur de fichiers avec une latence élevée est accélérée.
...
### <a name="skype-for-business-non-security-updates"></a>Skype Entreprise : mises à jour non relatives à la sécurité
-   Ajout d’une boîte de dialogue expliquant pourquoi un utilisateur ne parvient pas à participer à une réunion lorsque certains ports sont bloqués ou que certaines adresses IP ne sont pas mises en liste verte.
-   Résolution d’un problème : les messages non lus des salles de conversation permanentes étaient marqués comme lus lorsque vous cliquiez sur les onglets de conversation par messagerie instantanée.
-   Résolution d’un problème : les toasts de messagerie instantanée entrants présentaient un retard de plusieurs secondes.
...
-   Résolution d’un problème : le nombre d'invités ne s’affichait pas correctement pour l’organisateur des réunions lorsque celles-ci dépassaient la limite.
-   Résolution d’un problème : le numéro de téléphone n’était pas affiché dans le toast pour les appels PSTN entrants.
-   Résolution d’un problème : quand vous utilisiez la touche Suppr lors de la modification du nom d'un groupe de la liste de contacts, le groupe entier était supprimé.
...
-   Résolution d’un problème : les caractères non anglais dans une conversation et un historique de conversations étaient tronqués.
...
-   Ajout d’un paramètre intrabande autorisant la restriction « Tout le monde (aucune restriction) » en tant qu’option pour « Ces personnes ne doivent pas patienter dans la salle d’attente. »
-   Ajout de la possibilité d’activer ou désactiver la vidéo automatique pour les appels vidéo P2P dans VDIv2.
-   Résolution d’un problème : les numéros des contacts apparaissaient en double dans le menu déroulant d’appel.
-   Résolution d’un problème : les délégués étaient supprimés pour les utilisateurs qui passaient de Skype Entreprise à Skype Entreprise Basic.
-   Résolution d’un problème : l’option Apparaître hors ligne était absente lorsque vous utilisiez les stratégies client d’activation de l’option Apparaître en ligne et la personnalisation d’URL d’état.
-   Agrandissement du bouton Participer à une réunion pour corriger la troncation du texte dans certaines langues localisées.
-   Augmentation de la visibilité des messages d’importance haute dans une discussion.
-   Ajout des types d’extension de fichier Office et Skype Entreprise aux listes de blocage de transfert de fichier autorisées.
-   Corrections de localisation dans les invitations de réunion Outlook pour le texte de pied de page de réunion dans une autre langue que l'anglais.
-   Résolution d’un problème : les noms d’expéditeurs pouvaient être intervertis dans les conversations entre plusieurs utilisateurs.
-   Résolution d’un problème : la fenêtre de conversation vide ne s’affichait pas tant que l'utilisateur n’avait pas rejoint la réunion avec succès.
-   Résolution d’un problème : les informations du champ Département dans une carte professionnelles étaient vides dans les résultats de recherche si le champ de titre était vide.
...
-   Résolution d’un problème : les compléments COM ne recevaient pas les événements d’un document ouvert lorsqu’un fichier Visio était ouvert en double-cliquant sur une icône de fichier ou un nom de fichier.
...
-   **Activité de fichier partagé :** cliquez sur le bouton Activité dans le coin supérieur droit du fichier pour voir à quel moment un fichier partagé dans OneDrive Entreprise ou SharePoint a été partagé, modifié, renommé ou restauré.
-   **Liens fiables :** quand un utilisateur clique sur un lien, la protection avancée contre les menaces d’Office 365 inspecte le lien pour déterminer s’il est malveillant. Si le lien est considéré comme malveillant, l’utilisateur est redirigé vers une page d’avertissement au lieu de l’URL cible d’origine. [Plus d’informations](https://support.office.com/article/dd6a1fef-ec4a-4cf4-a25a-bb591c5811e3)
-   **Modifications de fichiers partagés :** affichez l’auteur des modifications dans les documents partagés et restaurez les versions antérieures. [Plus d’informations](https://support.office.com/article/5c1e076f-a9c9-41b8-8ace-f77b9642e2c2)
...
-   [CVE-2018-0807](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0807) : Vulnérabilité d’exécution de code à distance Microsoft Word
-   [CVE-2018-0812](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2018-0812) : vulnérabilité de corruption de mémoire de Microsoft Word
-   [ADV170020](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/ADV170020) : mise à jour de protection fiable pour Microsoft Office
-   [ADV170021](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/ADV170021) : mise à jour de protection fiable pour Microsoft Office

### <a name="word-non-security-updates"></a>Word : mises à jour non relatives à la sécurité
-   Résolution d’un problème : Word se bloquait lorsqu’un utilisateur essayait d’utiliser l’option Enregistrer sous pour un document existant dans OneDrive Entreprise, puis annulait l’enregistrement ou tentait de fusionner les modifications existantes.
-   Résolution d’un problème : lors de l’enregistrement d’un fichier synchronisé, Office ne parvient pas à écrire sur un disque, mais continue le chargement du fichier sur OneDrive. Avec ce correctif, un message d’erreur s’affiche désormais et le chargement ne s’effectue pas.
-   Résolution d’un problème : Word pouvait se bloquer si l’utilisateur accédait à l’onglet Insertion peu après l'ouverture de Word.
...
-   Résolution d’un problème : dans certaines conditions, Word se bloquait lorsque vous tentiez de récupérer des fichiers issus du cloud.
...
### <a name="office-suite-non-security-updates"></a>Suite Office : mises à jour non relatives à la sécurité
-   Ajoutez la prise en charge de l’authentification unique (SSO) pour les utilisateurs de domaine pour les plans Office 365 Allemagne où l’identité est fédérée avec un Active Directory local.
...
-   Résolution d’un problème : le zoom et la mise à l’échelle ne fonctionnaient pas correctement dans les compléments Office sous environnement dynamique PPP.
-   Résolution d’un problème : le nœud CurrentStatus du fournisseur de services de configuration Office (CSP) renvoyait une chaîne vide même si Office 365 ProPlus était installé.
-   Résolution d’un problème : le format de fichier .box était modifié, ce qui avait une incidence sur le fonctionnement des versions antérieures d’Office installées sur le même ordinateur, car les fichiers .box sont utilisés par toutes les versions d’une application Office installée sur un même ordinateur.
...
-   Résolution d’un problème : les propriétés des fichiers Office n’étaient pas affichées dans l’Explorateur de fichiers.
...
-   Résolution d’un problème : certains modules VBA dont le nom contenait des caractères codés sur deux octets ne pouvaient pas être ouverts.
-   Résolution d’un problème : la boîte de dialogue Quoi de neuf ? ne s’affichait pas.
-   Résolution d’un problème : lorsque vous cliquiez sur l’onglet Dessiner, l’application se bloquait pour certains utilisateurs.
-   Résolution d’un problème : lorsque vous placiez le curseur de la souris sur un contrôle commun qui comportait une info-bulle, l’application se bloquait.
...
-   Résolution d’un problème : le mode de signature de certains fichiers programmes était incorrect, ce qui engendrait le signalement de ces derniers par des programmes antivirus et causait des problèmes de protection ou d’accès à des données dans le cadre de la protection des informations Windows (WIP).
...
-   Amélioration de l’accessibilité des contrôles utilisés dans mscomctl.ocx.
