...
## <a name="version-2008-april-13"></a>Version 2008 : 13 avril
...
- Nous avons résolu un problème qui provoquait une fermeture inattendue de certains utilisateurs lors de la synchronisation de leur boîte aux lettres.


### <a name="powerpoint"></a>PowerPoint

- Nous avons résolu un problème qui faisait que les utilisateurs voyaient les applications Office ne pas répondre suite à l'affichage d'une carte de visite et à la navigation vers une autre application.


### <a name="word"></a>Word

- Résolution d'un problème pour lequel le bouton intégré de l'étiquette de confidentialité est grisé lors de l'enregistrement d'un document Word et de sa réouverture.



[//]: # (NE PAS SUPPRIMER LA FIN DU CONTENU BUGDETAILS)

## <a name="version-2002-april-13"></a>Version 2002 : 13 avril
...
- Résolution d’un problème qui entraînait la fermeture inattendue de l’application lors de l’utilisation de la fenêtre Zoom.


- Ce correctif corrige les problèmes d’exécution de certaines requêtes qui ont précédemment généré le message d’erreur « La requête est trop complexe ».


- Cette modification résout un problème qui pouvait entraîner la fermeture inattendue d’Access lors du lancement de la zone Zoom (Maj + F2) pour modifier le texte.
...
- Résolution d’un problème pour vous permettre d’appeler le type de données date/heure étendue dans votre code sans rencontrer de problèmes dans votre application.
...
- Nous avons résolu un problème d’erreur qui se produisait lors de la tentative d’utilisation du générateur DSN ODBC
...
- Nous avons résolu un problème qui pouvait se produire lorsque vous tentiez de créer une connexion de données si vous vous étiez déconnecté de votre compte.


- Correction d’un bogue avec des API PivotDateFilter dans lequel les conditions de filtre « Avant » et « Après » étaient inversées.


- Résolution d’un problème qui provoquait la fermeture inattendue de Excel lorsque vous essayiez d’insérer des tableaux croisés dynamiques dans une feuille de graphique.


- Résolution d’un problème lié aux tableaux croisés dynamiques qui ne pouvaient pas être modifiés et aux classeurs qui ne pouvaient pas être enregistrés s’ils étaient exportés à partir d’un plan Project.


- Une erreur peut se produire lorsque vous essayez d’enregistrer un fichier qui contient une formule qui utilise la fonction LET().
...
- Résolution d’un problème lié aux livres en lecture seule qui n’actualisaient plus les données du tableau croisé dynamique lorsqu’ils étaient ouverts.


- Cette modification fournit un correctif pour le problème suivant : la fonctionnalité Excel « Insérer un objet » n’affiche pas l’icône correcte lors de l’insertion d’un fichier à partir du dossier de synchronisation local OneDrive.
...
- Résolution d’un problème concernant les clients, qui recevaient une notification incorrecte à propos d’une nouvelle version du fichier lors de la co-création.
...
- Résolution d’un problème qui pouvait survenir lors de l’utilisation de données en temps réel, en conjonction avec la fonctionnalité de recalcul multithread.
...
- Résolution d’un problème qui provoquait la fermeture inattendue de Excel dans certaines circonstances lors de l’utilisation de la fonctionnalité Reproduire la mise en forme.
...
- Résolution d’un problème qui provoquait la fermeture inattendue de Excel lors de l’utilisation de l’Analyse rapide après avoir figé la ligne supérieure de la feuille.
...
- Résolution d’un problème qui entraînait la fermeture inattendue d’Excel dans certains cas après la copie d’une feuille de calcul contenant un tableau croisé dynamique.


- Résolution d’un problème lié au lien externe, qui cesse de fonctionner une fois le fichier rouvert, si le chemin d’accès du fichier est trop long.


- Résolution d’un problème qui pouvait entraîner la fermeture inattendue de l’application lors de l’utilisation de données en temps réel, en conjonction avec la fonctionnalité de recalcul multithread.


- Résolution d’un problème concernant les liens externes, qui n’étaient pas mis à jour lors du remplissage si le livre source était fermé.


- Résolution d’un problème entraînant un avertissement sur la corruption d’un classeur s’il contenait des formules utilisant SI.NON.DISP().
...
- Amélioration de la synchronisation et de la stabilité du service en modifiant temporairement la fréquence de création des historiques de versions de page.
...
- Nous avons résolu un problème concernant le menu Orthographe, qui ne se chargeait pas.


### <a name="outlook"></a>Outlook

- Nous avons résolu un problème concernant les compléments web qui ne se chargeaient pas, car ils étaient bloqués par des expériences connectées facultatives.  Si vous souhaitez en savoir plus, consultez cet article :  https://developer.microsoft.com/en-us/office/blogs/outlook-add-ins-and-optional-connected-experiences/.
...
- Nous avons résolu un problème concernant l’échec du retour de résultats de recherche dans les calendriers de Groupe.


- Corrige un problème qui entraînait des fermetures occasionnelles lorsque les utilisateurs interagissaient avec des pièces jointes dans le cloud.


- Résolution d’un problème en raison duquel les utilisateurs de CLP rencontraient un problème lors du passage de l’adresse d’expéditeur d’une réponse d’un contexte protégé à un contexte non protégé.
...
- Nous avons résolu un problème qui pouvait survenir lors de la suppression de 4 courriers électroniques ou plus à partir d’un compte POP avec l’option « Télécharger les en-têtes uniquement » sélectionnée.
...
- Nous avons résolu un problème que les utilisateurs pouvaient rencontrer lors de l’attribution d’applications MAPI tierces.


- Nous avons résolu un problème qui provoquait la fermeture inattendue de Outlook lors de l’ouverture de fichiers.msg ou .oft enregistrés en local après une mise à jour Windows.


- Résolution d’un problème qui provoquait la fermeture inattendue d’Outlook sur certaines versions de Windows.
...
- Résolution d’un problème concernant Outlook, qui invitait sans cesse les utilisateurs a exécuter l’outil de réparation de la boîte de réception.


- Résolution d’un problème qui entraînait un blocage occasionnel lorsque les utilisateurs utilisaient le bouton « X » de leur souris.
...
- Nous avons résolu un problème à l’origine du brouillage des en-têtes des messages chinois lors de la réponse ou du transfert.
...
- Nous avons résolu un problème concernant les options Chiffrer uniquement et Ne pas transférer, qui continuaient d’être activées dans certains scénarios malgré leur désactivation par la stratégie.


- Nous avons résolu un problème de perte de format des SmartLinks, lorsqu’ils étaient enregistrés sous forme de brouillons.
...
- Nous avons résolu un problème concernant les caractères chinois, qui sont remplacés par des points d’interrogation lors de l’enregistrement sous la forme d’un fichier OFT.


- Nous avons résolu un problème à l’origine de fermetures inattendues lors de la récupération des informations personnelles.


- Résout un problème qui entraînait des fermetures occasionnels lorsque les utilisateurs modifiaient les destinataires.
...
- Correction d’un problème qui entraînait une fermeture inattendue pour certains utilisateurs lors de l’envoi de commentaires d’une notification d’administrateur.
...
- Nous avons résolu un problème concernant le complément de contenu Forms, qui ne s’affichait pas après l’insertion, jusqu’à ce que l’utilisateur clique sur une autre diapositive pour l’afficher.


- Correctif de sécurité pour résoudre un problème qui désactivait les protections de gestion des droits relatifs à l’information lors de l’ouverture d’un fichier PowerPoint en mode protégé.


- Nous avons résolu un problème VBA concernant le Slide.Shapes.AddMediaObject2, qui se fermait de manière inattendue avec les anciens formats vidéo (MPG-1,Mpeg-2).


- Nous avons résolu un problème qui provoquait la conversion incorrecte d’un graphique Excel lié en feuille Excel lorsque l’utilisateur modifiait le chemin d’accès source vers le dossier OneDrive local.


- Résolution d’un problème qui pouvait survenir lorsqu’un fichier contenait des commentaires actuels et anciens, ce qui déclenchait une mise à niveau des commentaires.


- Nous avons résolu un problème lié au volet de suggestions qui pouvait entraîner la fermeture inattendue de l’application.


- Nous avons résolu un problème dans la boîte de dialogue Paramètres de l’action qui provoquait la fermeture inattendue de l’application PowerPoint.


- Nous avons résolu un problème lié à l’application PowerPoint qui pouvait entraîner sa fermeture inattendue.
...
- Nous avons résolu un problème concernant le diaporama en mode protégé, qui ne fonctionnait pas.
...
- Résolution d’un problème qui provoquait le déclenchement d’un ProjectBeforeTaskChangeevent supplémentaire lorsque les données du Prédécesseur ou du Successeur étaient modifiées en mode Formulaire.


- Résolution d’un problème lié à Project, qui se bloquait lors de l’enregistrement de projets créés avec d’anciennes versions de Project.


- Résolution d’un problème lié au travail de référence chronologique, qui ne pouvait être saisi lorsque le paramètre de protection du travail réel était activé.
...
- Résolution d’un problème lié à Project, qui pouvait s’arrêter de manière inattendue à l’ouverture des fichiers dans lesquels les contours de ressources ont été spécifiés d’une certaine manière.
...
- Résout un problème lié au certificat TLS-DSK d’un utilisateur, qui ne se renouvelait pas à l’heure prévue, mais uniquement lorsqu’il restait moins de 12 heures de validité.


- Résout un problème lié à l’interface utilisateur de Skype Entreprise, qui était vide après la connexion lorsqu’Office n’était pas encore sous licence.


### <a name="visio"></a>Visio

- Nous avons résolu un problème à l’origine de la fermeture inattendue de l’Aperçu instantané lors de l’alignement du texte.
...
- Résolution d’un problème que les utilisateurs pouvaient rencontrer lors de la réponse à un nouveau message ou de la rédaction de celui-ci.


- Résolution d’un problème qui entraînait un blocage occasionnel lorsque les utilisateurs utilisaient le bouton « X » de leur souris.


- Résolution d’un problème qui pouvait survenir lors de l’ouverture de courriers électroniques très volumineux.


- Nous avons résolu un problème lié aux en-têtes numérotés collés, qui affichaient un retrait supplémentaire.
...
- Nous avons résolu un problème qui pouvait provoquer un arrêt inopiné lors de l’ouverture d’un document.


- Nous avons résolu un problème lié aux liens longs, qui n’étaient pas encapsulés lors de l’enregistrement d’un document au format HTML.


- Nous avons résolu un problème lié aux styles de base, qui n’étaient pas mis à jour avec le style Normal.


- Corrige un bogue lié aux extensions de commentaire dans lesquelles la réponse au commentaire aurait la même extension que le commentaire parent.
...
- Résolution d’un problème qui pouvait entraîner la fermeture inattendue de l’application lors du démarrage.


- Nous avons résolu un problème concernant le statut de message Ne pas transférer sous Outlook.


- Nous avons résolu un problème lié à la boîte de dialogue Galerie de styles.


- Nous avons résolu un problème lié à l’ouverture automatique des macros, qui s’exécutait avant l’exécution automatique.


### <a name="office-suite"></a>Suite Office

- Résolution d’un problème pour l’ancien volet de partage non basé sur un service web, qui pouvait entraîner la fermeture inattendue de l’application si le document était fermé alors que le volet de partage était ouvert.


- Résolution d’un problème de synchronisation qui pouvait entraîner la fermeture inattendue de l’application lors de la fermeture de fichiers Office.
...
- Cette modification résout un problème lié au proxy de secours SVG qui entraînait des violations d’accès.


- Corrige l’utilisation élevée du processeur en mode veille avec les images GIF/modèle 3D animé.
...
- Cette modification corrige un problème qui pouvait survenir lors du lancement des applications Office en raison de l’échec de chargement du fichier d2d1.dll.


- L’hôte Office cesse de fonctionner de manière inattendue dans Windows lorsqu’un complément est activé alors que la clé de Registre HKEY_CURRENT_USER \Software\Microsoft\Internet Explorer\Main\TabProcGrowth est définie sur zéro. Cette modification devrait résoudre ce problème.


- Résolution d’un problème de fonctionnement de l’API de messagerie pour les compléments Office.
...
- Résolution d’un problème qui pouvait être à l’origine d’une fermeture inattendue lors du déplacement de contenu à partir de l’application.
...
- Corrige un bogue concernant les extensions de commentaire dans lesquelles la réponse au commentaire aurait la même extension que le commentaire parent.
